...
// import { WhatsApp, LocationOn, Phone } from '@mui/icons-material';
...
const OrderCard: React.FC = () => {
  ...
  const [orderDetails, setOrderDetails] = useState<OrderCardProps | null>(null);
  ...
  // Calcul du total
  const finalPrice =
    orderDetails?.items.reduce((total: number, item: any) => {
      return total + item.product.price * item.quantity;
    }, 0) || 0;

  const handleCompleteOrder = async () => {
    await axios.patch(`http://localhost:3300/orders/${id}`, {
      status: 'COMPLETED',
    });
  };
  const handleCanseleeOrder = async () => {
    await axios.patch(`http://localhost:3300/orders/${id}`, {
      status: 'CANCELED',
    });
  };
  ...
  return orderDetails ? (
    <div className="w-full h-screen flex flex-col items-center justify-center">
      ...
      <div className="bg-white rounded-2xl p-5 shadow-md max-w-[500px] my-5">
        ...
        <div className="flex justify-between items-center mb-5">
          ...
          {orderDetails.status === 'ACTIVE' ? (
            <span className="bg-green-50 text-green-500 px-3 py-1.5 rounded text-sm">Активный</span>
          ) : orderDetails.status === 'COMPLETED' ? (
            <span className="bg-[#F3F4F6] text-[#333] px-3 py-1.5 rounded text-sm">
              Завершенный
            </span>
          ) : orderDetails.status === 'CANCELED' ? (
            <span className="bg-[#F3F4F6] text-[#333] px-3 py-1.5 rounded text-sm">Отмененный</span>
          ) : null}
        </div>
        ...
        {orderDetails.status === 'ACTIVE' && (
          <div className="flex gap-4">
            <button
              onClick={handleCanseleeOrder}
              className="flex-1 py-3 px-4 rounded-lg border-none text-base cursor-pointer bg-gray-100 text-gray-800"
            >
              Отменить
            </button>
            <button
              onClick={handleCompleteOrder}
              className="flex-1 py-3 px-4 rounded-lg border-none text-base cursor-pointer bg-green-500 text-white"
            >
              Завершить
            </button>
          </div>
